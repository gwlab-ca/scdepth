          ...
          [](const std::string&              baseline_file,
             const std::vector<std::string>& files) -> py::dict {
              ...
              for(uint32_t i = 0; i < go.steps; ++i) {
                  ...
                  out[py::str("stability_{}").format(i)] =
                      move_vec_1d<double>(std::move(m.stability));
                  out[py::str("stability_1_{}").format(i)] =
                      move_vec_1d<double>(std::move(m.stability_1));
                  out[py::str("stability_2_{}").format(i)] =
                      move_vec_1d<double>(std::move(m.stability_2));
                  out[py::str("stability_3p_{}").format(i)] =
                      move_vec_1d<double>(std::move(m.stability_3p));
              }
