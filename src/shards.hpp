...
public:
    ...
    void init(const std::string& out_prefix, size_t max_elems,
              double write_frac) {
    ...
    size_t tags_written() const { return tags_written_; }

    size_t tags_cached() const { return tags_.size(); }

    GeneBreaks& gene_breaks() { return breaks_; }

    const GeneBreaks& gene_breaks() const { return breaks_; }

    size_t size() const { return shard_files_.size(); }

    size_t raw_molecules() const { return raw_molecules_; }
...
private:
    struct ShardFile {
        ...
        uint32_t barcode() const { return next.bc.barcode; }
    ...
    long int write_one_(BGZF* bout, const std::string& fname,
    ...
    bool update_break_(int tid, int lft) {
        ...
        while(bidx_ < breaks_[tid].size() &&
              static_cast<int>(breaks_[tid][bidx_]) < lft) {
            past_break = true;
            bidx_++;
        }
        return past_break;
