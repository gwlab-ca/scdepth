...
void BarcodeCounter::init(
    const std::string& lib_string, bool fwd, const std::string& barcode_tag,
    ...
    if(fwd)
        strand_ = StrandMode::TAG_FWD;
    else
        strand_ = StrandMode::TAG_REV;
    barcode_tag_[0]       = barcode_tag[0];
    barcode_tag_[1]       = barcode_tag[1];
    umi_tag_[0]           = umi_tag[0];
    umi_tag_[1]           = umi_tag[1];
    ...
    if(samples.empty() != sample_tag.empty()) {
        std::cerr
            << "sample ids and sample_tag must both be empty or specified\n";
        exit(1);
    }
    sample_tag_[0] = sample_tag[0];
    sample_tag_[1] = sample_tag[1];
    samples_       = samples;
    ...
    // std::cout << "barcode tag = " << barcode_tag << " len = " <<
    // barcode_length << " regex = " << barcode_regex_str_ << "\n";
    try {
        barcode_re_ = std::regex(barcode_re,
                                 std::regex::ECMAScript | std::regex::optimize);
    } catch(const std::regex_error& e) {
        std::cerr << "Error compiling barcode regex " << e.what()
                  << " code = " << e.code() << " regex = " << barcode_re
                  << "\n";
        exit(1);
    }
    ...
    if(!random_hex_regex_str_.empty()) {
        try {
            random_hex_re_ = std::regex(
                random_hex_re, std::regex::ECMAScript | std::regex::optimize);
        } catch(const std::regex_error& e) {
            std::cerr << "Error compiling random_hex_re_ regex " << e.what()
                      << " code = " << e.code() << " regex = " << random_hex_re
                      << "\n";
            exit(1);
        }
    }
...
void BarcodeCounter::set_count_parameters(
    double min_gene, unsigned int min_gene_bases, double min_exonic,
    unsigned int min_intronic, uint8_t min_qual, bool discard_unknown_juncs,
    bool probes) {
...
bool BarcodeCounter::prepare_bam(const std::string& gtf, const std::string& in,
                                 const std::string& out, int threads,
                                 size_t max_tags, double max_tag_frac) {
...
unsigned int scdepth::block_overlaps(const std::vector<Exon>& blocks,
    ...
    while(eit != exons.end() && it != blocks.end()) {
        if(eit->rgt < it->lft) {
            ...
            bases +=
                std::min(it->rgt, eit->rgt) - std::max(it->lft, eit->lft) + 1;
            if(it->rgt < eit->rgt) {
            ...
            std::cout << "    This should never happen block = " << it->lft
                      << " - " << it->rgt << " exon = " << eit->lft << " - "
                      << eit->rgt << "\n";
            exit(1);
        }
...
unsigned int scdepth::check_junctions(const std::vector<Exon>& blocks,
...
bool extract_flag_from_qname(std::string_view qname, const std::regex& pattern,
                             std::string_view true_value, bool& error) {
...
size_t BarcodeCounter::process_reads(size_t chunk) {
    ...
    while(processed < chunk) {
        ...
        if((barcode_length_ != 0 && cb_len != barcode_length_) ||
           umi_len != umi_length_) {
            full_.inc(TagSummary::BAD_TAGS, false, false);
            // std::cout << "bad tag length cb = " << cb_len << " vs " <<
            // barcode_length_
        ...
        if(!sample_set_.empty()) {
            ...
            if(sample == NULL ||
               sample_set_.find(sample) == sample_set_.end()) {
                full_.inc(TagSummary::BAD_TAGS, false, false);
                continue;
            }
        ...
        if(track_primer_type) {
            ...
            random_hex =
                extract_flag_from_qname(bam_get_qname(rec), random_hex_re_,
                                        random_hex_value_, hex_error);
            if(hex_error) {
                full_.inc(BAD_TAGS, false, false);
                if(samp_sum != nullptr)
                    samp_sum->inc(TagSummary::BAD_TAGS, false, false);
                continue;
            }
            full_.inc_pa_hex(TOTAL_READS, true, random_hex);
            // increment the total reads since they weren't counted above
            if(samp_sum != nullptr)
                samp_sum->inc_pa_hex(TagSummary::TOTAL_READS, true, random_hex);
        }
        ...
        if(!umi_okay) {
            ...
            if(samp_sum)
                samp_sum->inc(TagSummary::BAD_TAGS, track_primer_type,
                              random_hex);
            continue;
        }
        ...
        if(barcode_counts_.size() <= bidx)
            barcode_counts_.resize(bidx + 1);
        barcode_counts_[bidx].total++;
        if(sample && barcode_counts_[bidx].sample.empty()) {
            barcode_counts_[bidx].sample = sample;
        }

        if(rec->core.tid < 0 || rec->core.flag & BAM_FUNMAP ||
           rec->core.qual < min_qual_) {
            full_.inc(TagSummary::LOW_QUALITY, track_primer_type, random_hex);
            if(samp_sum)
                samp_sum->inc(TagSummary::LOW_QUALITY, track_primer_type,
                              random_hex);
            continue;
        }
        ...
        // std::cout << "Read overlaps = " << overlaps.size() << " xs = " << xs
        // << " pos = " << lft << " - " << rgt << "\n";
        ...
        if(has_probes_) {
            ...
            if(GX == NULL || strchr(GX, ';') != NULL) {
                ...
                if(samp_sum)
                    samp_sum->inc(TagSummary::NO_GENE, track_primer_type,
                                  random_hex);
                continue;
            } else {
                ...
                if(git != gene_map_.end()) {
                    ...
                    full_.inc(TagSummary::NO_GENE, track_primer_type,
                              random_hex);
                    if(samp_sum)
                        samp_sum->inc(TagSummary::NO_GENE, track_primer_type,
                                      random_hex);
            ...
            full_.inc(TagSummary::AMBIGUOUS_READS, track_primer_type,
                      random_hex);
            full_.inc(TagSummary::COUNTABLE_READS, track_primer_type,
                      random_hex);
            if(samp_sum) {
                samp_sum->inc(TagSummary::COUNTABLE_READS, track_primer_type,
                              random_hex);
                samp_sum->inc(TagSummary::AMBIGUOUS_READS, track_primer_type,
                              random_hex);
            }
        ...
        for(auto o : overlaps) {
            ...
            // std::cout << "  Potential gene " << g.gene_id << " strand = " <<
            // g.strand << " pos = " << g.lft << " - " << g.rgt << "\n";
            /*
            if(read_blocks_.size() > 1){
                std::cout << " Junctions passed = " <<
            check_junctions(read_blocks_, g->junctions);
            }
            std::cout << " junctions: ";
            ...
            */
            ...
            for(auto& b : read_blocks_) {
                if(b.overlaps(g.lft, g.rgt)) {
                    gbases +=
                        (std::min(b.rgt, g.rgt) - std::max(b.lft, g.lft)) + 1;
                    in_gene++;
                }
            }
            if(gbases < min_gene_bases_ ||
               (1.0 * gbases / rbases) < min_gene_ ||
               in_gene != read_blocks_.size())
                continue;
            ...
            if(read_blocks_.size() > 1) {
                ...
                if(discard_unknown_juncs_ &&
                   jcnt != (read_blocks_.size() - 1)) {
                    continue;
                }
            }
            ...
            std::cout << std::fixed << std::setprecision(3)
                << "    gid = " << g.gidx << " gene_id = " << g.gene_id << "
            gbases = " << gbases
                << " in_gene = " << in_gene << " gene frac = " << (1.0 * gbases
            / rbases)
                << " ioverlaps = " << ioverlaps << " blocks = " <<
            read_blocks_.size() << " efrac = " << min_exonic_
                << "\n";
            */
        ...
        if((scount + acount) == 1) {
            ...
            if(scount == 1) {
                ...
                full_.inc(TagSummary::SPLICED_READS, track_primer_type,
                          random_hex);
                if(samp_sum)
                    samp_sum->inc(TagSummary::SPLICED_READS, track_primer_type,
                                  random_hex);
            } else {
                ...
                full_.inc(TagSummary::AMBIGUOUS_READS, track_primer_type,
                          random_hex);
                if(samp_sum)
                    samp_sum->inc(TagSummary::AMBIGUOUS_READS,
                                  track_primer_type, random_hex);
            }
        } else if((scount + acount) == 0 && ucount == 1) {
            tag.make_tag(bidx, ugidx, umi, 0, 1, random_hex);
            full_.inc(TagSummary::UNSPLICED_READS, track_primer_type,
                      random_hex);
            ...
            if(samp_sum) {
                samp_sum->inc(TagSummary::COUNTABLE_READS, track_primer_type,
                              random_hex);
                samp_sum->inc(TagSummary::UNSPLICED_READS, track_primer_type,
                              random_hex);
            }
            barcode_counts_[bidx].countable++;
            if(random_hex)
...
bool BarcodeCounter::finish() {
    ...
    if(!shards_.merge_shards(barcode_counts_)) {
        std::cerr << "[errror] Error merging " << shards_.size()
                  << " shard files\n";
        return false;
    }
...

    {
        ...
        gzo << "barcode\tbarcode_idx\ttotal_reads\tcountable_reads\traw_"
               "molecules\toffset\ttotal_bytes\tsample\tcountable_random_"
               "hex\tcountable_poly_A\n";
        ...
        for(auto& b : bseqs) {
            ...
            gzo << b << "\t" << bidx << "\t" << barcode_counts_[bidx].total
                << "\t" << barcode_counts_[bidx].countable << "\t"
...

    {
        ...
        if(!random_hex_regex_str_.empty()) {
            for(size_t i = 0; i < TagSummary::TOTAL_FIELDS; i++) {
                os << "polyA_" << TagNames[i] << "\treads\t"
                   << full_.polyA_counts[i] << "\n";
            }
            for(size_t i = 0; i < TagSummary::TOTAL_FIELDS; i++) {
                os << "random_hex_" << TagNames[i] << "\treads\t"
                   << full_.random_hex_counts[i] << "\n";
            }
        }
        os << "raw_molecules\treads\t" << shards_.raw_molecules() << "\n";
        os << "bam\tparam\t" << bam_file_ << "\n";
        os << "min_gene\tparam\t" << std::fixed << std::setprecision(4)
           << min_gene_ << "\n";
        os << "min_exonic\tparam\t" << std::fixed << std::setprecision(4)
           << min_exonic_ << "\n";
        os << "min_gene_bases\tparam\t" << min_gene_bases_ << "\n";
        os << "min_intronic\tparam\t" << min_intronic_ << "\n";
        os << "min_qual\tparam\t" << static_cast<int>(min_qual_) << "\n";
        os << "discard_unknown_juncs\tparam\t"
           << static_cast<int>(discard_unknown_juncs_) << "\n";
    ...
    if(!samples_.empty()) {
        ...
        for(auto& s : samples_) {
            ...
            for(size_t i = 0; i < TagSummary::TOTAL_FIELDS; i++) {
                os << s << "_" << TagNames[i] << "\treads\t"
                   << ss.merged_counts[i] << "\n";
            }
            if(!random_hex_regex_str_.empty()) {
                for(size_t i = 0; i < TagSummary::TOTAL_FIELDS; i++) {
                    os << s << "_" << "polyA_" << TagNames[i] << "\treads\t"
                       << ss.polyA_counts[i] << "\n";
                }
                for(size_t i = 0; i < TagSummary::TOTAL_FIELDS; i++) {
                    os << s << "_" << "random_hex_" << TagNames[i]
                       << "\treads\t" << ss.random_hex_counts[i] << "\n";
                }
            }
        }
    }
