...
bool TagShards::write(){
    ...
    outss << out_prefix_ << ".shard_tags." << std::setfill('0') << std::setw(4) << shard_count_ << ".gz";
    ...
    if (mit != merged_.end()) {
        std::cerr << "[error] merged had leftover CountTags after final barcode write\n";
        bgzf_close(bout);
        return false;
    }
...
long int TagShards::write_one_(BGZF * bout, const std::string & fname, const BarcodeHeader & bc,
        std::vector<UMITag>::const_iterator & mit)
{
    if(bgzf_write(bout, reinterpret_cast<const void*>(&bc), sizeof(BarcodeHeader)) <= 0){
    ...
    for(size_t i = 0; i < bc.n_count_tags; i++){
        ...
        const auto & m = *mit;
        if(bgzf_write(bout, reinterpret_cast<const void*>(&(m)), sizeof(UMITag)) <= 0){
            std::cerr << "[error] Error writing to shard file " << fname << "\n";
            bgzf_close(bout);
            return -1;
        };
    ...
    if(written != bc.block_size){
        std::cerr << "[error] block_size mismatch for barcode = "
            << bc.barcode << " tag count = " << bc.n_count_tags 
            << " expected written = " << (sizeof(UMITag) * bc.n_count_tags)
            << " header = " << bc.block_size
            << " written = " << written
            << "\n";
        bgzf_close(bout);
        return -1;
...
long int TagShards::write_one_(BGZF * bout, const std::string & fname, const BarcodeData & d){
    ...
    if(mit != d.counts.end()){
        std::cerr << "Error writing record for barcode " << d.bc.barcode << " count iterator is not at the end\n";
        return -1;
    }
    return written;
}
...
bool TagShards::merge_shards(std::vector<BarcodeCount> & bc_counts){
    ...
    if(in_memory && !merge_()) return false;
    std::cerr << "Merging " << size() << " shard files";
    if(in_memory) std::cerr << " and in memory tags\n";
    std::vector<UMITag>::const_iterator mit = merged_.cbegin();
    ...
    for(auto & f : shard_files_){
        ...
        if(ret <= 0){
            std::cerr << "[error] Error reading barcode from shard file " << f.file << "\n";
            close_all_();
            bgzf_close(bout);
            return false;
        }
    ...
    while(bc_min < std::numeric_limits<uint32_t>::max()){
        ...
        //std::cout << "merging barcode = " << bc_min << " detected = " << barcodes_detected << " / " << bc_counts.size() << "\n";
        for(auto & f : shard_files_){
            if(f.done) continue;
            ...
            if(f.barcode() == bc_min){
                ...
                merged.counts.insert(merged.counts.end(), f.next.counts.begin(), f.next.counts.end());

                auto ret = read_barcode(f.fin, f.next);
                if(ret < 0){
                    std::cerr << "[error] Error reading barcode from shard tag file " << f.file << "\n";
                    close_all_();
                    bgzf_close(bout);
                    return false;
                }else if(ret == 0){
        ...
        if(in_memory && bit != barcodes_.end()){
            ...
            if(bit->barcode == bc_min){
                ...
                for(size_t i = 0; i < bit->n_count_tags; i++){
                    if(mit == merged_.end()){
                        std::cerr << "[error] Error merging in memory barcode count (unexpected end)" << bit->barcode << "\n";
                        close_all_();
                        bgzf_close(bout);
                        return false;
                    }
        ...
        if(written != expected){
            std::cerr << "[error] Bytes mismatch written = " << written << " expected = " << expected << " block size = " << merged.bc.block_size << "\n";
            return false;
        }
        bc_min = bc_next;
    ...
    if(in_memory){
        if(bit != barcodes_.end()){
            std::cerr << "[error] Error in memory barcodes were not fully merged\n";
    ...
    for(auto & f : shard_files_){
        if(!f.done){
            error = true;
            std::cerr << "[error] Error shard " << f.file << " barcodes were not fully merged\n";
        }
    }
    close_all_();
    ...
    for(auto & f : shard_files_){
        try{
            ...
            std::cout << "[error] File system error removing file " << f.file << "  error = " << err.what() << '\n';
        }
    }
    /*
    for(size_t i = 0; i < bc_counts.size(); i++){
        const auto & b = bc_counts[i];
        std::cerr << "barcode = " << i << " total = " << b.total << " countable = " << b.countable << " offset = " << b.offset << "\n";
