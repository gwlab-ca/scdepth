...
DownsamplerThread::DownsamplerThread(Downsampler& data, uint32_t umi_len,
                                     size_t genes, size_t chunk_start,
                                     size_t chunk_end)
    : data_(data), cstart_(chunk_start), cend_(chunk_end), umi_len_(umi_len) {
    out.reset(data.output.fracs, data.output.barcodes, genes,
              data.output.max_hist, data.output.build_mats, data.has_visium,
              data.output.calc_sau, data.aggregate_only, data.samples);
}
...
void DownsamplerThread::operator()() {
    ...
    for(size_t i = cstart_; i < cend_; i++) {
        ...
        if(bgzf_seek(fin, data_.barcodes[i].offset, SEEK_SET) < 0) {
            std::cerr << "[error] Error seeking to "
                      << data_.barcodes[cstart_].offset << " for " << tagf
                      << "\n";
            error = true;
            bgzf_close(fin);
            return;
    ...
    // std::cout << " starting thread " << cstart_ << " - " << cend_ << " use qc
    ...
    for(size_t i = cstart_; i < cend_; i++) {
        ...
        if(bc.countable == 0) {
            if(out.build_mats) {
                for(size_t i = 0; i < data_.output.fracs.size(); i++) {
                    if(out.calc_sau) {
                        out.spliced_mat[i].indptr.push_back(
                            out.spliced_mat[i].data.size());
                        out.unspliced_mat[i].indptr.push_back(
                            out.unspliced_mat[i].data.size());
                        out.ambiguous_mat[i].indptr.push_back(
                            out.ambiguous_mat[i].data.size());
                        if(out.spliced_mat[i].indptr.size() !=
                           (bc.index - cstart_ + 2)) {
                            std::cerr << "[error] indptr size = "
                                      << out.spliced_mat[i].indptr.size()
                                      << " c = " << i << " cstart = " << cstart_
                                      << " index = " << bc.index << "\n";
                        }
                    }
                    out.total_mat[i].indptr.push_back(
                        out.total_mat[i].data.size());
                    if(out.total_mat[i].indptr.size() !=
                       (bc.index - cstart_ + 2)) {
                        std::cerr << "[error] indptr size = "
                                  << out.total_mat[i].indptr.size()
                                  << " c = " << i << " cstart = " << cstart_
                                  << " index = " << bc.index << "\n";
                    }
                }
            ...
            if(!data_.aggregate_only) {
                for(size_t i = 0; i < data_.output.fracs.size(); i++) {
                    out.gcounts[i].indptr.push_back(
                        out.gcounts[i].indices.size());
                }
            }
            continue;
        }
        if(read_barcode(fin, b_) <= 0) {
            std::cerr << "[error] Error reading barcode c = " << i << " ("
                      << cstart_ << " - " << cend_ << ") " << bc.barcode
                      << " index = " << bc.index
                      << " bgzf_tell = " << bgzf_tell(fin)
                      << " offset = " << bc.offset << "\n";
            error = true;
            bgzf_close(fin);
            return;
        ...
        for(auto & c : b_.counts){
            std::cerr << "    tag  gene = " << c.gene << " reads = " <<
        c.total()
        ...
        */
        ...
        for(size_t i = 0; i < o.fracs.size(); i++) {
            corrector.downsample(o.fracs[i], umi_len_, bc, b_, bco,
                                 data_.umi_directed, data_.umi_multi,
                                 data_.primer_mode); //, data_.umi_singletons);
            ...
            std::cout << "  barcode = " << bc.index
                ...
                << " unspliced indptr size = " <<
            out.unspliced_mat[i].indptr.size()
                << " ambiguous indptr size = " <<
            out.ambiguous_mat[i].indptr.size()
                << "\n";
            */
            /*
            if(out.spliced_mat[i].indptr.size() != (bc.index - cstart_ + 1)){
                std::cerr << "[error] indptr size = " <<
            out.spliced_mat[i].indptr.size() << " c = " << c << " cstart = " <<
            cstart_ << " index = " << bc.index << "\n";
            }
            if(out.unspliced_mat[i].indptr.size() != (bc.index - cstart_ + 1)){
                std::cerr << "[error] indptr size = " <<
            out.unspliced_mat[i].indptr.size() << " c = " << c << " cstart = "
            << cstart_ << " index = " << bc.index << "\n";
            }
            if(out.ambiguous_mat[i].indptr.size() != (bc.index - cstart_ + 1)){
                std::cerr << "[error] indptr size = " <<
            out.ambiguous_mat[i].indptr.size() << " c = " << c << " cstart = "
            << cstart_ << " index = " << bc.index << "\n";
            }
            */
        }
...
void DownsamplerThread::merge_barcode_(size_t i, const BarcodeCount& bc,
                                       const UMIResults& bco) {
    // std::cout << "don't forget to keep track of the number of reads lost due
    // to ambiguous umi/genes\n"; merge the stuff
    ...
    uint32_t bc_spliced_mols = 0, bc_unspliced_mols = 0, bc_ambiguous_mols = 0,
             bc_total_mols    = 0;
    uint32_t bc_spliced_reads = 0, bc_unspliced_reads = 0,
             bc_ambiguous_reads = 0, bc_total_reads = 0;
    while(it != bco.umis.end()) {
        uint32_t spliced_mols = 0, unspliced_mols = 0, ambiguous_mols = 0,
                 total_mols    = 0;
        uint32_t spliced_reads = 0, unspliced_reads = 0, ambiguous_reads = 0,
                 total_reads = 0;
        auto start           = it;
