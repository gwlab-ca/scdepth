...
struct SpinLock {
    ...
    void reset() noexcept { f.clear(std::memory_order_relaxed); }
};
...
inline bool bz_write_vector(BGZF* bout, const std::vector<T>& v) {
    ...
    if(bgzf_write(bout, reinterpret_cast<const void*>(&v[0]), sizeof(T) * sz) <=
       0) {
        return false;
    };
    return true;
...
template <typename T> inline bool bz_read_vector(BGZF* bin, std::vector<T>& v) {
    uint64_t sz = 0;
    ...
    if(sz > 0) {
        if(bgzf_read(bin, reinterpret_cast<void*>(&v[0]), sizeof(T) * sz) <=
           0) {
            return false;
        }
    }
