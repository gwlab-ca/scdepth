...
struct DirectedEdges {
    inline void operator()(size_t i, size_t j, UMINode& n1,
                           UMINode& n2) const noexcept {
...
void UMIDirectional::downsample(
    double frac, uint32_t umi_len, const BarcodeCount& bc,
    ...
    size_t tot = 0, otot = 0;
    */
    // std::cout << "  Downsampling barcode " << bc.barcode << " frac = " <<
    // frac << " raw mols = " << bc.raw_molecules  << "\n"; std::cout << " check
    // counts = " << bd.counts.size() << "\n";
    out.umis.clear();
    out.dis_reads = 0;
    out.dis_ambig = 0;
    out.dis_mols  = 0;
    ...
    while(it != bd.counts.end()) {
        auto start = it;
        // std::cout << "    downsampling gene " << start->gene << " index = "
        // << std::distance(bd.counts.begin(), start) << " / " <<
        // bd.counts.size() << "\n";
        n_nodes_ = 0;
        // s  nodes = 0;
        while(it != bd.counts.end() && start->gene == it->gene) {
            if((primer_mode == PrimerMode::PolyAOnly && it->is_random_hex()) ||
               (primer_mode == PrimerMode::RandomHexOnly &&
                !it->is_random_hex())) {
                it++;
                continue;
            }
            ...
            if((spliced + ambiguous + unspliced) > 0) {
                if(n_nodes_ >= nodes_.size())
                    nodes_.resize(n_nodes_ + 5);
            ...
            /*std::cout << "    n_nodes_ = " << n_nodes_ << " gene = " <<
               start->gene
        ...
        // std::cout << "    downsampled gene " << start->gene << " number of
        // nodes = " << n_nodes_  << "\n";
        if(n_nodes_ == 0)
            continue;
        if(n_nodes_ < 500) {
            if(directional)
                build_graph_brute_(DirectedEdges{});
        } else {
            if(directional)
                build_graph_hash_(DirectedEdges{});
        }
        correct_umis_(start->gene, out);
        // lost += (n_nodes_ - out.umis.size());
        // std::cout << "    after gene = " << start->gene << " raw = " <<
        // n_nodes_
        //     << " corrected = " << out.umis.size() << " lost so far = " <<
        //     lost << "\n";
        /*
        if(final_umis_.size() != n_nodes_){
            std::cout << "    after gene = " << start->gene << " raw = " <<
        n_nodes_
                << " corrected = " << final_umis_.size() << " lost so far = " <<
        lost << "\n";
        }
        lost += (n_nodes_ - final_umis_.size());
        processed += n_nodes_;
        */
        // out.umis.insert(out.umis.end(), final_umis_.begin(),
        // final_umis_.end());
    }
...
template <typename T> void UMIDirectional::build_graph_brute_(T build_edge) {
    ...
    for(size_t i = 0; i < n_nodes_; i++) {
        ...
        for(size_t j = i + 1; j < n_nodes_; j++) {
            ...
            if(n1.is_random_hex() != n2.is_random_hex())
                continue;
...
template <typename T> void UMIDirectional::build_graph_hash_(T build_edge) {
    umi_map_.clear();
...
void UMIDirectional::correct_umis_(uint32_t gene, UMIResults& out) {
    ...
    std::sort(idx_.begin(), idx_.end(), [this](uint32_t a, uint32_t b) {
    ...
    // if(n_nodes_ >= 100) std::cout << "  umi components for gene = " << gene
    // << " = " << comps_full_.size() << "\n";
    for(size_t i = 0; i < comps_full_.size(); i++) {
        auto& c = comps_full_[i];
        if(c.empty())
            continue;
        // if(n_nodes_ >= 100) std::cout << "    component = " << i << " size =
        // " << c.size() << "\n";
        std::sort(c.begin(), c.end(), [this](uint32_t a, uint32_t b) {
            if(nodes_[a].total() != nodes_[b].total())
                return nodes_[a].total() > nodes_[b].total();
            return nodes_[a].umi < nodes_[b].umi;
        });
...
void UMIDirectional::correct_multi_umis_(
    UMIResults& out) { //, bool profile_singles){
    ...
    "\n"; for(auto & u : out.umis){ std::cout << "    corrected umi = " << u.umi
    << " count = " << u.count << "\n";
    }
    */
    ...
    while(start != out.umis.end()) {
        ...
        if(all_bad) {
            out.dis_reads +=
                start->total(); // only if you define ties as “discard all”
            out.dis_mols++;
            start->set_invalid();
        }
        if(has_bad)
