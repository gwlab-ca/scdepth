...
template <typename T> py::array move_vec_1d(std::vector<T>&& v) {
    auto* vec = new std::vector<T>(std::move(v));

    py::capsule owner(
        vec, [](void* p) { delete reinterpret_cast<std::vector<T>*>(p); });
...
void scdepth::bind_vis_bins(py::module_& m) {
    m.def(
        ...
        [](const Downsampler& ds, uint32_t step, uint32_t row_div,
           uint32_t col_div) -> py::tuple {
            VisiumBin out;
            {
                py::gil_scoped_release release;
                out =
                    scdepth::aggregate_visium_bins(ds, step, row_div, col_div);
            }
            ...
            if(!out.mt_mols.empty())
                b["mt_molecules"] =
                    move_vec_1d<uint32_t>(std::move(out.mt_mols));
            if(!out.mod_mols.empty())
                b["mod_molecules"] =
                    move_vec_1d<uint32_t>(std::move(out.mod_mols));
        ...
        py::arg("downsampler"), py::arg("step"), py::arg("row_div"),
        py::arg("col_div"),
        "Aggregate per-barcode outputs into Visium HD bins. Returns a dict of "
        "1d numpy arrays.");
    ...
    m.def("aggregate_visium_hists",
            ...
            unsigned int min_molecules, bool in_tissue) -> py::dict
        {
            {
                py::gil_scoped_release release;
                scdepth::aggregate_visium_hists(ds, out, step, min_molecules,
    in_tissue);
            }
    ...
    py::return_value_policy::reference);
        ...
        //py::arg("frac") = 0.0,
        //py::arg("seed") = 0,
        "Aggregate barcode molecule count hists by row and col.  The last last
    histogram bin(ie the tail) is not included."
    );

    m.def("aggregate_visium",
        [](const Downsampler& ds, RowColSums& out, uint32_t step,
            unsigned int min_molecules, bool in_tissue, bool permute, double
    frac, uint32_t seed) -> py::dict
        {
            {
                py::gil_scoped_release release;
                scdepth::aggregate_visium(ds, out, step, min_molecules,
    in_tissue, permute, frac, seed);
            }
    ...
    py::return_value_policy::reference);
        ...
        py::arg("frac") = 0.0,
        py::arg("seed") = 0,
        "Aggregate barcode molecule counts by row and col with optional
    permuting or bootstrapping."
    );
    */
}
