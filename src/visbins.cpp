...
void bin_genes(const GeneCountMatrix&       gmat,
               const std::vector<uint64_t>& barcode2bin, size_t n_bins,
    ...
    for(size_t i = 0; i < n_barcodes; i++)
        bin_ptr[barcode2bin[i] + 1]++;
    for(size_t i = 1; i < bin_ptr.size(); i++)
        bin_ptr[i] += bin_ptr[i - 1];
    ...
    for(size_t bin = 0; bin < n_bins; ++bin) {
        ...
        for(size_t i = begin; i < end; ++i) {
            ...
            if(barcode >= n_rows) {
                // std::cerr << "error barcode " << barcode << " is greater than
                // " << n_rows << "\n";
                continue;
            }
            // merge gene ids from each barcode in the bin
            auto g_start =
                std::next(gmat.indices.begin(), gmat.indptr[barcode]);
            auto g_end =
                std::next(gmat.indices.begin(), gmat.indptr[barcode + 1]);
        ...
        */
        // std::cout  n_bins = " << n_bins << " bin = " << bin << " size = " <<
        // bc_genes.size() << "\n";
        genes[bin] = std::distance(gidx.begin(), uend);
    }
}

void scdepth::bin_gene_counts(const Downsampler& ds, uint32_t step,
                              uint32_t bin_div, SparseMatrix& out) {
    auto&                 dso    = ds.output;
    ...
    for(size_t bin = 0; bin < n_bins; ++bin) {
        ...
        for(size_t i = begin; i < end; ++i) {
            ...
            auto g_end =
                std::next(mat.indices.begin(), mat.indptr[barcode + 1]);
...
bool scdepth::make_filt_gbin_mat(const BinnedMap&             bm,
                                 const GeneCountMatrix&       gmat,
                                 GeneCountMatrix&             go,
...
bool scdepth::make_filt_cbin_mat(const BinnedMap& bm, const SparseMatrix& gmat,
                                 SparseMatrix&                go,
                                 const std::vector<uint32_t>& idx) {
    ...
    for(auto bin : idx) {
        ...
        for(size_t i = begin; i < end; ++i) {
            ...
            for(size_t j = gmat.indptr[barcode]; j < gmat.indptr[barcode + 1];
                j++) {
...
VisiumBin scdepth::aggregate_visium_bins(const Downsampler& ds, uint32_t step,
                                         uint32_t row_div, uint32_t col_div) {
    VisiumBin out;
    out.invalid = true;
    auto& dso   = ds.output;
    if(ds.bin_pos.empty() || ds.barcodes.size() != ds.bin_pos.size()) {
        std::cerr << "[error] the bin positions must be specified and the same "
                     "size as `barcdoes`";
        return out;
    }
    ...
    if(dso.has_mt)
        out.mt_mols.assign(N, 0);
    if(dso.has_mod)
        out.mod_mols.assign(N, 0);
    ...
    for(size_t i = 0; i < dso.barcodes; i++) {
        ...
        if(dso.has_mt)
            out.mt_mols[idx] += dso.total.bc_mt_mols[didx];
        if(dso.has_mod)
            out.mod_mols[idx] += dso.total.bc_mod_mols[didx];
...
/*
void scdepth::aggregate_visium_hists(const Downsampler & ds, RowColHists & out,
uint32_t step, unsigned int min_molecules, bool in_tissue)
{
    auto & dso = ds.output;
    out.resize(dso.barcodes, ds.total_rows, ds.total_cols, dso.max_hist,
dso.has_mt); const size_t H = out.max_hist - 1; uint32_t * rows =
out.rows.data(); uint32_t * cols = out.cols.data(); const uint32_t * mh =
ds.output.cell_mhist.data(); for(size_t i = 0; i < dso.barcodes; i++){ size_t r
= ds.bin_pos[i].r; size_t c = ds.bin_pos[i].c;
        //size_t p = out.pidx.empty() ? i : out.pidx[i];
        size_t p = i;
        size_t mols = ds.output.total.bc_mols[size_t(step) * dso.barcodes + p];
        size_t reads = ds.output.total.bc_reads[size_t(step) * dso.barcodes +
p]; if(mols < min_molecules || (in_tissue && !ds.bin_pos[i].in_tissue)){
            continue;
        }
        ...
        if(dso.has_mt){
            size_t mt = ds.output.total.bc_mt_mols[size_t(step) * dso.barcodes +
p]; out.row_mt_molecules[r] += mt; out.col_mt_molecules[c] += mt;
        }
...
void scdepth::aggregate_visium(const Downsampler & ds, RowColSums & out,
uint32_t step, unsigned int min_molecules, bool in_tissue, bool permute, double
frac, uint32_t seed)
{
    auto & dso = ds.output;
