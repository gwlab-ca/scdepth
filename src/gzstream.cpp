...
std::streambuf* gzfilebuf::setbuf(char_type* p, std::streamsize n) {
    ...
    // needs at least a buffer of size 1 (very inefficient though, therefore
    // make it bigger?) This follows from [27.5.2.4.3]/12 (gptr needs to point
...
void gzfilebuf::enable_buffer() {
    ...
    if(own_buffer && !buffer) {
        ...
        if(buffer_size > 0) {
            ...
            // Get area starts empty and will be expanded by underflow as need
            // arises
            this->setg(buffer, buffer, buffer);
            // Setup entire internal buffer as put area.
            // The one-past-end pointer actually points to the last element of
            // the buffer, so that overflow(c) can safely add the extra
            // character c to the sequence. These pointers remain in place for
        ...
        // If buffer already allocated, reset buffer pointers just to make sure
        // no stale chars are lying around
        this->setg(buffer, buffer, buffer);
        this->setp(buffer, buffer + buffer_size - 1);
    }
}
...
gzofstream::gzofstream(const std::string& name, std::ios_base::openmode mode)
    : std::ostream(NULL), sb() {
    this->init(&sb);
