/*
 * this file is from Heng Li's cgranges repository
 * https://github.com/lh3/cgranges/tree/master
 ...
 */
template <typename S, typename T> // "S" is a scalar type; "T" is the type of
    ...
    int                   index_core(std::vector<Interval>& a) {
        ...
        for(i = 0; i < a.size(); i += 2)
            last_i = i, last = a[i].max = a[i].en; // leaves (i.e. at level 0)
        for(k = 1; 1LL << k <= static_cast<long long int>(a.size());
            ++k) { // process internal nodes in the bottom-up order
            size_t x = 1LL << (k - 1), i0 = (x << 1) - 1,
                   step = x << 2; // i0 is the first node
            for(i = i0; i < a.size();
                i += step) { // traverse all nodes at level k
                S el = a[i - x].max; // max value of the left child
                S er     = i + x < a.size() ? a[i + x].max
                                                              : last; // of the right child
                S e      = a[i].en;
                e        = e > el ? e : el;
                e        = e > er ? e : er;
            ...
            last_i = last_i >> k & 1
                                           ? last_i - x
                                           : last_i + x; // last_i now points to the parent of the
                                       // original last_i
            if(last_i < a.size() &&
               a[last_i].max > last) // update last accordingly
                last = a[last_i].max;
        }
        return k - 1;
    }
...
public:
    ...
    bool overlap(const S& st, const S& en, std::vector<size_t>& out) const {
        ...
        if(max_level < 0)
            return false;
        stack[t++] =
            StackCell(max_level, (1LL << max_level) - 1,
                      0); // push the root; this is a top down traversal
        while(t) { // the following guarantees that numbers in out[] are always
                   // sorted
            StackCell z = stack[--t];
            if(z.k <= 3) { // we are in a small subtree; traverse every node in
                           // this subtree
                size_t i, i0 = z.x >> z.k << z.k,
                          i1 = i0 + (1LL << (z.k + 1)) - 1;
                if(i1 >= a.size())
                    i1 = a.size();
                ...
                size_t y =
                    z.x -
                    (1LL << (z.k - 1)); // the left child of z.x; NB: y may be
                                        // out of range (i.e. y>=a.size())
                stack[t++] =
                    StackCell(z.k, z.x, 1); // re-add node z.x, but mark the
                                            // left child having been processed
                if(y >= a.size() ||
                   a[y].max > st) // push the left child if y is out of range or
                                  // may overlap with the query
                    stack[t++] = StackCell(z.k - 1, y, 0);
            } else if(z.x < a.size() &&
                      a[z.x].st < en) { // need to push the right child
                if(st < a[z.x].en)
                    out.push_back(z.x); // test if z.x overlaps the query; if
                                        // yes, append to out[]
