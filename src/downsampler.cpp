...
bool Downsampler::init(const std::string& prefix, const std::string& mt_prefix,
                       const std::string& mt_file, const std::string& mod_file,
                       const std::string& exclude_file, size_t max_hist,
                       bool build_matrices, bool calc_sau) {
    ...
    if(!barcode2sample.empty() && barcode2sample.size() != barcodes.size()) {
        std::cerr << "barcode2sample size mismatch " << barcode2sample.size()
                  << " vs expected = " << barcodes.size() << "\n";
        return false;
    }
    ...
    if(!mt_prefix.empty() && !mt_file.empty()) {
        ...
        for(size_t i = 0; i < genes.size(); i++) {
            if(genes[i].gene_name.compare(0, mt_prefix.size(), mt_prefix) ==
               0) {
    ...
    countable_ = std::accumulate(barcodes.begin(), barcodes.end(), 0,
                                 [](uint32_t sum, const BarcodeCount& bc) {
                                     return sum + bc.countable;
                                 });

    raw_molecules_ = std::accumulate(barcodes.begin(), barcodes.end(), 0,
                                     [](uint32_t sum, const BarcodeCount& bc) {
                                         return sum + bc.raw_molecules;
                                     });

    return true;
}

bool Downsampler::init_visium(const std::vector<uint32_t>& rows,
                              const std::vector<uint32_t>& cols,
                              std::vector<uint32_t>&       in_tissue,
                              std::vector<uint32_t>&       countable_reads,
    ...
    if(barcodes.size() != rows.size() || barcodes.size() != cols.size() ||
       in_tissue.size() != barcodes.size() ||
       countable_reads.size() != barcodes.size() ||
       total_reads.size() != barcodes.size()) {

        std::cerr << "barcodes/rows/cols all must be the same size for "
                     "rows/cols and empty or the same size for "
                     "countable_reads/total_reads/in_tissue"
...
bool Downsampler::downsample(std::vector<double>& fracs, uint32_t umi_len,
                             uint64_t seed, unsigned int threads,
                             bool aggregate_only, const std::string& umi_mode,
    ...
    if(threads < 1)
        threads = 1;
    if(threads > barcodes.size())
        threads = barcodes.size();
    ...
    if(!has_visium)
        reset_visium();
    output.reset(fracs, barcodes.size(), genes_, max_hist_, build_mats_,
                 this->calc_sau_, !aggregate_only, has_visium, samples);
    output.aggregate_only = aggregate_only;
    // if visium output is specified get it ready
    if(total_rows > 0 && total_cols > 0) {
        has_visium = true;
    }
    // std::cout << "total rows = " << total_rows << " cols = " << total_cols <<
    // " use qc = " << use_qc << "\n";
    ...
    */
    ...
    for(size_t i = 0; i < threads; i++) {
        workers.push_back(new DownsamplerThread(*this, umi_len_, genes_,
    ...
    for(size_t i = 0; i < workers.size(); i++) {
        ...
        // handle error
        if(w->error) {
            std::cerr << "[error] with worker " << workers[i]
                      << " chunk = " << chunks_[i] << " - " << chunks_[i + 1]
                      << "\n";
            error = true;
        }
    }
    ...
    if(build_mats_ && calc_sau_){
        for(size_t i = 0; i < fracs.size(); i++){
            for(size_t j = 0; j < barcodes.size(); j++){
                ...
                size_t mat_sum = sum_sparse(barcodes[j].index,
    output.spliced_mat[i]); size_t mgenes = output.spliced_mat[i].indptr[j + 1]
    - output.spliced_mat[i].indptr[j]; if(expected != mat_sum){ std::cout <<
    "sparse spliced matrix error for frac = " << i << " j = " << j
                        << " spliced mat barcode= " << barcodes[j].index
                        << " expected = " << expected
                        << " matrix sum = " << mat_sum
                        << " delta = " << ((long int)expected - (long
    int)mat_sum)
                        << " reads = " << output.spliced.bc_reads[bidx]
                        << " genes = " << output.spliced.bc_genes[bidx]
                        << " mgenes = " << mgenes
                        << " delta = " << ((long
    int)output.spliced.bc_genes[bidx] - (long int)mgenes)
                        << "\n";
                    return true;
                }

                expected = output.unspliced.bc_mols[bidx];
                mat_sum = sum_sparse(barcodes[j].index,
    output.unspliced_mat[i]); mgenes = output.unspliced_mat[i].indptr[j + 1] -
    output.unspliced_mat[i].indptr[j]; if(expected != mat_sum){ std::cout <<
    "sparse unspliced matrix error for frac = " << i << " j = " << j
    ...
    int)mat_sum)
                        ...
                        << " mgenes = " << mgenes
                        << " delta = " << ((long
    int)output.unspliced.bc_genes[bidx] - (long int)mgenes)
                        << "\n";
                    return true;
                }
                ...
                mat_sum = sum_sparse(barcodes[j].index,
    output.ambiguous_mat[i]); mgenes = output.ambiguous_mat[i].indptr[j + 1] -
    output.ambiguous_mat[i].indptr[j]; if(expected != mat_sum){ std::cout <<
    "sparse ambiguous matrix error for frac = " << i << " j = " << j
    ...
    int)mat_sum)
                        ...
                        << " mgenes = " << mgenes
                        << " delta = " << ((long
    int)output.ambiguous.bc_genes[bidx] - (long int)mgenes)
                        << "\n";
                    return true;
                }
            }
...
bool Downsampler::write_gene_mats(const std::string&           out,
    ...
    if(idx.back() >= output.barcodes) {
        std::cerr << "[error] idx contains elements that exceed the number of "
                     "barcodes\n";
        return false;
    }
    ...
    if(bgzf_write(bout, reinterpret_cast<const void*>(&output.seed),
                  sizeof(uint64_t)) <= 0) {
    ...
    if(bgzf_write(bout, reinterpret_cast<const void*>(&N), sizeof(uint32_t)) <=
       0) {
    ...
    for(auto& g : output.gcounts) {
        ...
        if(bin_div > 0) {
            ...
            for(auto& i : idx) {
                go.indices.insert(
                    go.indices.end(), std::next(g.indices.begin(), g.indptr[i]),
...
bool Downsampler::write_gene_baseline(const std::string&           out,
                                      const std::vector<uint32_t>& idx,
    ...
    if(step >= output.fracs.size()) {
        std::cerr
            << "[error] Step cannot be larger than the number of fractions\n";
        return false;
    }
    ...
    if(bin_div > 0) {
        ...
        for(auto& i : idx) {
            go.indices.insert(go.indices.end(),
                              std::next(g.indices.begin(), g.indptr[i]),
                              std::next(g.indices.begin(), g.indptr[i + 1]));
            go.data.insert(go.data.end(),
                           std::next(g.data.begin(), g.indptr[i]),
