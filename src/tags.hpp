...
struct RawTag {
    ...
    void make_tag(uint32_t barcode, uint32_t gene, uint32_t umi, bool spliced,
                  bool unspliced, bool random_hex) {
    ...
    bool is_spliced() const { return (flag & F_SPLICED) != 0; }

    bool is_unspliced() const { return (flag & F_UNSPLICED) != 0; }

    bool is_random_hex() const { return (flag & F_RANDOM_HEX) != 0; }

    uint32_t unpack_random_hex() const { return is_random_hex() ? 1u : 0u; }

    uint32_t unpack_spliced() const { return is_spliced() ? 1u : 0u; }

    uint32_t unpack_unspliced() const { return is_unspliced() ? 1u : 0u; }
...
struct BarcodeCount {
    bool operator<(const BarcodeCount& rhs) const { return index < rhs.index; }
...
struct BarcodeHeader {
    void make_header(uint32_t barcode, uint32_t n_count_tags = 0,
...
struct UMITag {
    ...
    bool same_molecule(const UMITag& rhs) const {
        return gene == rhs.gene && umi == rhs.umi &&
...
enum StrandMode {
    TAG_UNKNOWN = 0,
    TAG_FWD = 1, // Tag maps to the forward strand // For 10X 3' R2 maps to the
                 // forward strand of the transcript
    TAG_REV = 2  // Tag maps to the reverse strand // For 10X 5' R2 maps to the
                 // reverse strand of the transcript
};

enum class PrimerMode { Merge, PolyAOnly, RandomHexOnly };
...
long int                  read_barcode_debug(BGZF* fin, BarcodeData& data,
