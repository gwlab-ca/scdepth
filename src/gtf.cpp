...
Genes gtf2genes_parse(const std::string & gtf, sam_hdr_t * hdr){
    ...
    while(getline(in, line)){
        ...
        if(parse_error){
            std::cerr << "[error] Error malformed line on line number " << line_no << " [" << line << "]\n";
            genes.clear();
            return genes;
        }
        ...
        if(fields[2] != "gene" && fields[2] != "exon") continue;
        int tid = -1;
        if(hdr != nullptr){
        ...
        if(tid < 0) continue;
        for(auto it = std::sregex_iterator(fields[8].begin(), fields[8].end(), words_regex);
        ...
        if(gene_id.empty()){
            std::cerr << "[error] Error missing gene_id on line number " << line_no << " [" << line << "]\n";
            genes.clear();
            return genes;
        }
        ...
        if(fields[2] == "gene"){
            ...
            auto tret = tx_map.insert(std::make_pair(tx_id, std::vector<Exon>()));
            tret.first->second.push_back(Exon(std::stoi(fields[3]) - 1, std::stoi(fields[4]) - 1));
    ...
    */
    for(size_t i = 0; i < genes.size(); i++){
        ...
        /*
        std::cout << "  tid = " << genes[i].tid << " pos = " << genes[i].lft << " - " << genes[i].rgt 
            << " id = " << genes[i].gene_id << " gidx = " << genes[i].gidx 
            << " exons = " << genes[i].exons.size() << " junctions = " << genes[i].junctions.size() 
            << "\n";
        */
    }
    return genes;
...
scdepth::RefTrees scdepth::genes2trees(const Genes & genes, unsigned int n_chroms){
    RefTrees trees;
...
scdepth::GeneBreaks scdepth::gene_breaks(const Genes & genes, unsigned int n_chroms, uint32_t delta){
    GeneBreaks ret;
    ret.resize(n_chroms);
    auto it = genes.begin();
    ...
    while(it != genes.end()){
        ...
        while(it != genes.end() && it->tid == start->tid){
            ...
            //std::cout << "  " << it->gene_id << " " << it->lft << " - " << it->rgt << "\n";;
            it++;
        }
...
GeneIDMap scdepth::read_gtf2mapping(const std::string & gtf_map){
    ...
    while (std::getline(in, line)) {
        ...
        if (!(iss >> gene.gidx
            ...
            std::cerr << "[error] failed to parse gtf mapping line from " << gtf_map << " line :\n" << line << "\n";
            ret.clear();
            return ret;
        }
        ret.push_back(gene);
...
void scdepth::set_gene_filter(const GeneIDMap & genes, const std::string & mod_file, std::vector<bool> & out, bool warn)
{
    ...
    while (std::getline(in, line)) {
        ...
        if(it != gidx_map.end()){
            ...
            std::cerr << "[warning] " << mod_file << " gene_id " << line << " is not found in the gtf index\n";
        }
    }
}
