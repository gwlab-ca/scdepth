...
static inline double mean_over_seeds(const std::vector<double>& v, size_t off,
                                     uint32_t seeds) {
    double sum = 0.0;
...
bool GeneOverlap::calculate() {
    ...
    for(size_t step = 0; step < steps; step++) {
        ...
        for(auto& s : seeds_) {
            if(!s.next()) {
                std::cerr << "[error] Error loading next seed for file "
                          << s.file << "\n";
            }
        }
        ...
        for(uint32_t bc = 0; bc < barcodes; bc++) {
            ...
            m.stability_3p[bc] =
                mean_over_seeds(sout_.stability_3p, off, seeds);
        }
    }
    return true;
}
...
bool GeneOverlap::SeedMats::init(const std::string& file) {
    ...
    if(bgzf_read(bin, reinterpret_cast<void*>(&barcodes), sizeof(barcodes)) <=
       0) {
...
bool GeneOverlap::init(const std::string&              baseline_file,
    ...
    if(!read_baseline_(baseline_file))
        return false;
    ...
    for(size_t i = 1; i < seeds_.size(); i++) {
        if(seeds_[i].barcodes != barcodes) {
            std::cerr << "[error] barcodes for seed and baseline (" << barcodes
                      << " vs " << seeds_[i].barcodes << ")\n";
            seeds_.clear();
            return false;
        }
        if(seeds_[i].genes != genes) {
            std::cerr << "[error] genes for seed and baseline (" << genes
                      << " vs " << seeds_[i].genes << ")\n";
            seeds_.clear();
            return false;
        }
        ...
        if(!vectors_equal(seeds_[i].fracs, seeds_[0].fracs)) {
            std::cerr << "[error] fractions for each seed are not equal ("
                      << seeds_[0].file << " vs " << seeds_[i].file << ")\n";
            seeds_.clear();
            return false;
        }
    }
