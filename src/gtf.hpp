...
struct Exon{
    ...
    Exon(uint32_t lft, uint32_t rgt)
        : lft(lft), rgt(rgt)
    {
    }
...
struct Gene{
    ...
    bool operator<(const Gene & rhs) const {
        return std::tie(tid, lft, rgt, gene_id) < std::tie(rhs.tid, rhs.lft, rhs.rgt, rhs.gene_id);
    }
...
GeneBreaks gene_breaks(const Genes & genes, unsigned int n_chroms, uint32_t delta = 5000);
...
void set_gene_filter(const GeneIDMap & genes, const std::string & mod_file, std::vector<bool> & out, bool warn = true);
