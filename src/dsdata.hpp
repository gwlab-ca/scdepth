...
template <typename T>
inline void sum_vectors(std::vector<T> & a, const std::vector<T> & b, bool optional_rhs){
    if(optional_rhs && b.empty()) return;
...
struct DownsampleResults{
    ...
    void set_cell_mhist(size_t f, size_t b, const std::vector<uint32_t> & mhist){
        size_t hs = get_hidx_start(f, b);
        assert(mhist.size() == max_hist);
